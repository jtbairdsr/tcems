 ...
 * @Last Modified time: 2015-05-01 11:07:52
 */
'use strict';
	...
	function($q, $alert, dataService) {
		...
		Data.prototype.remove = function(hideAlert) {
			...
			dataService.deleteItem(this.listName, this.Id)
				.success(function() {
					object.list = _.without(object.list, object);
					if (!hideAlert) {
						$alert({
							show: true,
							placement: 'top-right',
		...
		Data.queryFilter = function(listName, filter, expand) {
			var deffered = $q.defer();
			if (expand) {
				dataService.fetchItems(listName).top().where(filter).expand.execute()
					.success(function(data) {
						deffered.resolve(data.d.results);
					});
			} else {
				dataService.fetchItems(listName).top().where(filter).execute()
					.success(function(data) {
						deffered.resolve(data.d.results);
					});
			}
