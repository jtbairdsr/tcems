...
angular.module('employees').factory('Employee', function(
	$q, Data, Person, Employment, employees, currentSemester, areas,
	...
	Employee.prototype.propertyList = _.union(Person.prototype.propertyList, [
		'Active', 'Admin', 'Area/Id', 'INumber', 'Position/Id', 'Reader',
		'Retired', 'Team/Id', 'Track/Id'
	]);

	// Create the expandList
	Employee.prototype.expand = ['Area', 'Position', 'Team', 'Track'];
	...
	Employee.prototype.initAttributes = function() {
		...
		this.PositionId = (this.newData.PositionId) ? this.newData.PositionId : (
			(this.newData.Position) ? this.newData.Position.Id : undefined
		);
		this.Reader = this.newData.Reader || false;
		this.Retired = this.newData.Retired || false;
		this.TeamId = (this.newData.TeamId) ? this.newData.TeamId : (
			(this.newData.Team) ? this.newData.Team.Id : undefined
		);
		this.TrackId = (this.newData.TrackId) ? this.newData.TrackId : (
			(this.newData.Track) ? this.newData.Track.Id : undefined
		);
		...
		this.Position = (this.PositionId) ? _.find(positions.list, function(position) {
			return position.Id === object.PositionId;
		}) : undefined;
		this.Team = (this.TeamId) ? _.find(teams.list, function(team) {
			return team.Id === object.TeamId;
		}) : undefined;
		this.Track = (this.TrackId) ? _.find(tracks.list, function(track) {
		...
		this.Label = (this.Position) ? (
			'<div><img class="img-circle" src="' + this.Picture + '" fallback-src="src/modules/core/img/missing.png" width="50px" height="' + ((this.Position.Description === 'FTE') ? 70.8333 : 50) + 'px"> <b>' + this.PreferredName + ' ' + this.LastName + '</b></div>'
		) : (
			'<div><img class="img-circle" src="' + this.Picture + '" fallback-src="src/modules/core/img/missing.png"> <b>New Employee</b></div>'
		);
	 ...
	 */
	Employee.queryActive = function(returnArray) {
