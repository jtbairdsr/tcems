 ...
 * @Last Modified time: 2015-01-18 10:51:23
 */
/* global angular, _ */
...
(function() {
	...
	schedule.service('scheduleService', ['$location', 'generalService', 'dataService',
		function($location, generalService, dataService) {
			...
			service.scheduleService = function() {
				if (generalService.properties.currentUser.Position.Position === 'FTE' ||
					generalService.properties.currentUser.Position.Position === 'HR' ||
	...
	schedule.controller('ScheduleCtrl', ['$location', '$scope',
		'generalService', 'dataService',
		function($location, $scope, generalService, dataService) {
			var ctrl = this;
			...
			if (generalService.properties.currentUser.Position.Position !==
				'Coordinator' && generalService.properties.currentUser.Position
				.Position !== 'FTE' && !(generalService.properties.currentUser
					.Admin)) {
			...
			ctrl.refreshContent = function() {
				...
				new dataService.getItems('Availability')
					...
					.where({
						and: [
							['Current', 'eq', 1],
							['Employee/Id', 'eq', generalService.properties.currentUser
								.Id
							],
							['Semester/Id', 'eq', generalService.properties.currentSemester.Id]
						]
					})
					.execute(false)
					.success(function(data) {
					...
					function getSubShifts() {
						new dataService.getItems('SubShift')
							...
							.where({
								and: {
									or: [
										['Substitute', 'eq', generalService.properties.currentUser
											.Id
										],
										['Requester', 'eq', generalService.properties.currentUser
											.Id
										]
									],
									other: ['Active', 'eq', 1]
					...
					function getShifts() {
						new dataService.getItems('Shift')
							...
							.where({
								and: [
									['Current', 'eq', 1],
									['ShiftGroup/Id', 'eq', generalService.properties.currentSemester.ShiftGroup.Id]
								]
							})
							.execute()
							.success(function(data) {
								...
								getShifts = function() {
									new dataService.getItems('Shift')
										...
										.where({
											and: [
												['Current', 'eq', '1'],
												['ShiftGroup/Id', 'eq', generalService.properties.nextSemester.ShiftGroup.Id]
											]
										})
										.execute(false)
										.success(function(data) {
				...
				if (generalService.properties.nextSemester.ShiftGroup !== undefined) {
					...
					function newShift(subShift, shift) {
						...
						return {
							...
							hide: (subShift.Substitute.Id !== undefined &&
								!(generalService.properties.currentUser.Position.Position === 'FTE' ||
									generalService.properties.currentUser.Position.Position === 'HR' ||
									generalService.properties.currentUser.Admin)
							)
						};
	...
	schedule.controller('AvailabilityCtrl', ['$scope', 'generalService', 'dataService',
		'scheduleService',
		function($scope, generalService, dataService, scheduleService) {
			scheduleService.scheduleService();
			$scope.ctrlProperties.showAddShift = scheduleService.showAddShift;
			$scope.ctrlProperties.showAddSubShift = scheduleService.showAddSubShift;
			$scope.toggle = true;
			...
			function createSemesters() {
				var semesters = [];
				if (generalService.properties.nextSemester.ShiftGroup !== undefined) {
					generalService.properties.nextSemester.availabilityDays = createDays(generalService.properties.nextSemester);
				}
				generalService.properties.currentSemester.availabilityDays = createDays(generalService.properties.currentSemester);
				console.log(generalService.properties.currentSemester.availabilityDays);
			}
					...
					.success(function(data) {
						_.each(data.d.results, function(employee) {
							var submittedAvailabilities = _.find(availabilities, function(availability) {
								return availability.Employee.Id === employee.Id && availability.Semester.Id === generalService.properties.currentSemester.Id;
							});
	...
	schedule.controller('HrScheduleCtrl', ['$scope', '$alert', 'generalService', 'dataService',
		function($scope, $alert, generalService, dataService) {
			...
			ctrl.subRequest = {
				...
				ShiftId: '',
				RequesterId: generalService.properties.currentUser.Id,
				Date: Date.today()
			};
			new dataService.getItems('Shift')
				.top(999999999)
	...
	schedule.controller('TodayCtrl', ['$scope', 'generalService', 'dataService', 'scheduleService',
		function($scope, generalService, dataService, scheduleService) {
			var ctrl = this;
			scheduleService.scheduleService();
						...
						.where({
							and: [
								['Current',
									...
									'eq',
									generalService.properties.currentSemester.ShiftGroup.Id
								]
							]
						})
						.execute()
	...
	schedule.controller('MyScheduleCtrl', ['$scope', '$alert', 'generalService', 'dataService',
		function($scope, $alert, generalService, dataService) {
			var ctrl = this;
			ctrl.requestSub = function(shiftId, date) {
				var subRequest = {
					...
					RequesterId: generalService.properties.currentUser.Id,
					Date: date,
					SemesterId: generalService.properties.currentSemester.Id
				};
	...
	schedule.controller('SubShiftCtrl', ['$scope', '$alert', 'generalService', 'dataService',
		function($scope, $alert, generalService, dataService) {
			var ctrl = this;
			...
			ctrl.takeSubShift = function(subShiftId, shiftId, subShiftDate) {
				...
				var item = {
					...
					'SubstituteId': generalService.properties.currentUser.Id
				};
				new dataService.getItems('Schedule')
					...
					.where({
						and: [
							['Shift/Id', 'eq', shiftId],
							['Employee/Id', 'eq', generalService.properties.currentUser.Id
							],
							['Active', 'eq', 1]
						]
					})
						...
						if (data.d.results.length > 0) {
							...
							new dataService.getItems('SubShift')
								...
								.where({
									and: [
										['Shift/Id', 'eq', shiftId],
										['Substitute/Id', 'eq', generalService.properties.currentUser.Id
										],
	...
	schedule.controller('ShiftEditCtrl', ['$scope', '$alert', 'generalService', 'dataService',
		function($scope, $alert, generalService, dataService) {
			var ctrl = this,
				shift = $scope.shift,
				tmp = '';
			...
			ctrl.takeShift = function(shiftId) {
				var newSchedule = {
					...
					ShiftId: shiftId,
					EmployeeId: generalService.properties.currentUser.Id
				};
				new dataService.getItems('Schedule')
					.select(['Shift/Id', 'Employee/Id'])
					.expand(['Shift', 'Employee'])
	...
	schedule.controller('ShiftCreationCtrl', ['$scope', '$alert', 'generalService', 'dataService',
		function($scope, $alert, generalService, dataService) {
			var ctrl = this,
				newItem = {};
			$scope.newShift = {
	...
	schedule.controller('MyAvailabilityCtrl', ['$scope', '$alert', 'generalService', 'dataService',
		function($scope, $alert, generalService, dataService) {
			var ctrl = this;
	...
	schedule.controller('AddAvailability', ['$scope', '$alert', 'generalService', 'dataService',
		function($scope, $alert, generalService, dataService) {
			var ctrl = this;
			ctrl.newAvailability = {
				'__metadata': {
